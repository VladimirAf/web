<?php
define('EXTENSION_NAME', "Бланки печати");
define('EXTENSION_VERSION', "1.0.0");
define('EXTENSION_TYPE', "vQmod");
define('EXTENSION_COMPATIBILITY', "OpenCart 1.5.5 и 1.5.5.1");
define('EXTENSION_URL', "");
define('EXTENSION_SUPPORT_FORUM', "");

class ControllerModuleRupostBlanks extends Controller {
    private $error = array();

    private $defaults = array(
        'rpb_installed'              => 1,
        'rupost_blanks_status'   => 1,
        'rpb_from_name'          => '',
        'rpb_from_address_1'            => '',
        'rpb_from_address_2'        => '',
        'rpb_postindex'               => '',
    ...
    public function index() {
        $this->data = array_merge($this->data, $this->language->load('module/rupost_blanks'));
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
            ...
            $this->model_setting_setting->editSetting('rupost_blanks', $this->request->post);
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('module/rupost_blanks', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' :: '
        );

        $this->data['action'] = $this->url->link('module/rupost_blanks', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $this->template = 'module/rupost_blanks.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function install() {
        $this->load->model('setting/setting');
        $this->model_setting_setting->editSetting('rupost_blanks', $this->defaults);
        
    }

    public function uninstall() {
        $this->load->model('setting/setting');
        $this->model_setting_setting->deleteSetting('rupost_blanks');
    }

    private function validate() {
        if (!$this->user->hasPermission('modify', 'module/rupost_blanks')) {
